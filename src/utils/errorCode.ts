...
function genClass(errMeta: ErrorMeta) {
  ...
  return (
    ...
    if (opts?.noThrow) {
      try {
        ...
        handleException(e, errMeta.name, e.toString());
        return e;
      }
    } else {
 ...
 */
function errorTolerance(err: unknown): boolean {
  const builtinError = !(err instanceof BaseError) && config.errorToleranceFactor <= 6;
  const baseError = err instanceof BaseError && err.errorToleranceFactor < config.errorToleranceFactor;
