...
export class TaintManager {
  ...
  addPseudoTaintSender(sender: PseudoTaintSender) {
    ...
    console.log(sender.taintDef.uniqueId);
    console.log(this.getValidSenderTaintIds(this._contexts.get(sender.contextFilename)!, sender));
    console.log(this._contexts.get(sender.contextFilename)?.defToTaintIds.get(sender.taintDef.uniqueId));
   ...
   */
  /**
 * 修改后的 generateResolvedKey
 * 增加了 senderDefId 以区分携带相同 TaintID 的不同发送者
 */
private generateResolvedKey(
  ...
  // 获取 sender 的唯一标识：Def 的唯一 ID
  const senderDefId = sender.taintDef.uniqueId;

  // 如果你还想更精确，可以加上 sender 的 AST 位置
  const sRange = (sender.astNode as any)?.range;
  const senderLoc = sRange ? `${sRange[0]}:${sRange[1]}` : "unknown";

  // 最终的 Key 包含了：接收者标识 + 发送者上下文 + 发送者具体定义 + 污点ID
  return `${receiver.contextFilename}:${receiverIdentity}:${sender.contextFilename}:${senderDefId}:${senderLoc}:${senderTaintId}`;
}
  ...
  private _generateReportFromContext(
    ...
    const issues: any[] = [];
    const fileFrame = scriptUsageTracker.getPrimaryFrameByKey(ctx.filename);
    const fileFrameConstraint = scriptUsageTracker.getFrameConstraint(fileFrame);
    ...
    for (const ctx of this._contexts.values()) {
      ...
      for (const sink of ctx.sinks) {
        ...
        if (!flowSet.has(key)) {
          ...
          const sourceFrame = scriptUsageTracker.getPrimaryFrameByKey(sourceFile);
          const sourceFrameConstraint = scriptUsageTracker.getFrameConstraint(sourceFrame);
          const sourceFrames = scriptUsageTracker.getScriptFrameDescriptorsByKey(
            sourceFile,
          );
          const sinkFrame = scriptUsageTracker.getPrimaryFrameByKey(ctx.filename);
          const sinkFrameConstraint = scriptUsageTracker.getFrameConstraint(sinkFrame);
          ...
          if (shouldFilterSourceByFrame(source.sourceType, sourceFrame, sink.sinkType, sinkFrame)) {
            continue;
          }
