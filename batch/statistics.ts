...
import { BatchSummary, JobResult, StatisticsReport, TaintTypeStats } from "./types";

/**
 * Generate comprehensive statistics from batch results
 */
export function generateStatistics(summary: BatchSummary): StatisticsReport {
  ...
  const taintTypeMap = new Map<string, TaintTypeStats>();

  for (const result of results) {
    if (result.findings === 0) continue;
  ...
  const taintTypes = Array.from(taintTypeMap.values()).sort(
    (a, b) => b.count - a.count
  );
